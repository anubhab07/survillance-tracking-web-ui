...
export class SecurityService {
  ...
  requestSpecialAccess(visitorPayload) {
    const httpOptions = {
      headers: new HttpHeaders().set('Content-Type', 'application/json')
        .set('access-token', this.cookieService.get('token'))
    };

    return this.http.post(environment.dataUrl + '/web/addPoi', visitorPayload, httpOptions);
  }
  ...
  fetchSpecialVisitors() {
    ...
    return this.http.get(environment.dataUrl + '/web/getPoi', httpOptions);
  }

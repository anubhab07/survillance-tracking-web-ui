...
import { ISpecialGuestDetails } from '../Class/ISpecialGuestDetails';
import { ISpecialGuestListResponse } from '../Class/ISpecialGuestListResponse';
...
export class SecurityComponent implements OnInit {
  ...
  specialType = new FormControl('');
  specialTypeAccess: IVisitorTypeAccess[] = [
    { typeCode: 'VIP', typeDesc: 'VIP'},
    { typeCode: 'Missing', typeDesc: 'Missing'},
    { typeCode: 'Threat', typeDesc: 'Threat'},
  ];
  selectedSpecialType = this.specialTypeAccess[0].typeCode;
  specialGuestLst: ISpecialGuestDetails[] = [];
  ...
  ngOnInit() {
    ...
    this.securityService.fetchSpecialVisitors().subscribe((res: ISpecialGuestListResponse) => {
      ...
      res.data.forEach(guest => {
        guest.photo = this.sanitizer.bypassSecurityTrustResourceUrl('data:image/jpg;base64,' + guest.photo);
      });
      this.specialGuestLst = res.data;
      this.ngxLoader.stop();
    });
  }
  ...
  submitRequest() {
    if (this.selectedTab.value === 0) {
      ...
      if (this.email.valid && this.mobile.valid && this.name.valid && this.reffered.value && this.inTime.valid && this.outTime.valid && !this.photoUploadError) {
        ...
        if (visitorPayload.VisitorType === 'EMPLOYEE') {
          visitorPayload.Reffered = '0';
        }
    ...
    } else if (this.selectedTab.value === 1) {          // FOR SPECIAL TAB
      if (this.name.value && !this.photoUploadError) {
        const visitorPayloadLst = [];
        this.ngxLoader.start();
        const visitorPayload = {
          Name: this.newVisitor.name,
          Description: this.description.value,
          Type: this.specialType.value,
          Photo: this.newVisitor.photoId
        };
        visitorPayloadLst.push(visitorPayload);
        console.log(visitorPayloadLst);
        this.securityService.requestSpecialAccess(visitorPayloadLst).subscribe((response: IResponse) => {
          console.log(response);
          if (response.status === 1) {
            alert(response.message);
            this.initializeVisitor();
